...
$pdo = CONNECT_PDO();
$sql = "SELECT * FROM library WHERE UserID = ?";
...
foreach ($library as $ownedGame) {
    $sql = "SELECT * FROM games WHERE GameID = ?";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$ownedGame['GameID']]);
    ...
    <div class="sidebar">
        ...
        foreach ($games as $game) { ?>
            <div style="padding: 5px;">
                <a href="library.php?game=<?= $game['GameID'] ?>"><strong><?= $game['Title'] ?></strong></a>
            </div>
        <?php } ?>
    </div>
